...
import { normalizeAssistancePreferences, normalizeIncomeSources, normalizeIncomeRange, } from '../lib/profileFields.js';
...
router.get('/', requireAuth, async (req, res) => {
    ...
    const nextReportAt = estimateNextReportAt(data.last_report_at, data.report_frequency);
    res.json({
        profile: data,
...
router.patch('/', requireAuth, async (req, res) => {
    ...
    if (body.income_lower_inr !== undefined || body.income_upper_inr !== undefined) {
        const { data: current } = await supabaseAdmin
            ...
            .eq('id', userId)
            .single();
        const range = normalizeIncomeRange(body.income_lower_inr ?? current?.income_lower_inr ?? 5000, body.income_upper_inr ?? current?.income_upper_inr ?? 15000);
        if ('error' in range)
    ...
    res.json({
        profile: data,
        next_report_at: estimateNextReportAt(data.last_report_at, data.report_frequency)?.toISOString() ?? null,
    });
});
