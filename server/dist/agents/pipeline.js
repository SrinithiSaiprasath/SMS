...
function formatPeriodLabel(start, end, frequency) {
...
export async function runReportPipeline(userId, reportId) {
    await supabaseAdmin
        .from('reports')
...
export async function createReportForUser(userId) {
    const { data: profile } = await supabaseAdmin.from('users').select('*').eq('id', userId).single();
    if (!profile)
        throw new Error('Profile not found');
    const days = getPeriodDays(profile.report_frequency);
