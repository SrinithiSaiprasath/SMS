import { z } from 'zod';
import type { AgentAOutput, Category, UserProfile } from '../types/index.js';
import {
  buildCoachSkillsPrompt,
  buildEmailSkillsPrompt,
  buildReportSkillsPrompt,
  getEnabledSkillIds,
...
const AgentASchema = z.object({
  categorized_transactions: z.array(
    z.object({
...
export async function runAgentA(
  profile: UserProfile,
  transactions: Array<{ id: string; amount: number; description: string; raw_message: string; logged_at: string }>
...
Return ONLY valid JSON matching this structure:
{
  ...
  return AgentASchema.parse(JSON.parse(content)) as AgentAOutput;
}
...
export async function runAgentB(
  ...
  periodLabel: string,
  previousBreakdown?: { LEAK: number; WORTHY_ESSENTIAL: number; INVESTMENT: number } | null
): Promise<{ markdown: string; title: string; emailSections: Record<string, EmailSection> }> {
  const missionContext = buildMissionContextPrompt(profile);
  ...
  const enabledIds = getEnabledSkillIds(profile.agent_assistance_preferences).filter(
    (id) => id !== 'EMAIL_MISSION_REPORTS'
  );
