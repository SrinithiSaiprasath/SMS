...
export const AGENT_SKILLS = [
  {
    ...
    default: true,
    coachPrompt:
      'Classify every transaction as LEAK, WORTHY_ESSENTIAL, or INVESTMENT. Surface top leak patterns and rupee-saving opportunities within the user income range.',
    reportPrompt:
      'Always include: category summary, behavioral coaching blockquote, and top saving opportunities.',
  ...
  },
  {
    ...
    tier: 'core',
    coachPrompt:
      'Flag credit-related spend (EMIs, card swipes). Mention utilization impact on CIBIL when relevant.',
...
export const AGENT_ASSISTANCE_OPTIONS = AGENT_SKILLS.filter((s) => s.id !== 'EMAIL_MISSION_REPORTS').map(
  ({ id, label, tier, ...rest }) => ({
    id,
    label,
    tier,
...
export function normalizeSkillIds(preferences: string[] = []): string[] {
...
export function getEnabledSkillIds(preferences: string[] = []): AgentSkillId[] {
  return normalizeSkillIds(preferences) as AgentSkillId[];
}

export function getEnabledSkills(preferences: string[] = []) {
  const ids = new Set(getEnabledSkillIds(preferences));
  return AGENT_SKILLS.filter((s) => ids.has(s.id));
}

export function buildCoachSkillsPrompt(preferences: string[] = []): string {
...
export function buildReportSkillsPrompt(preferences: string[] = []): string {
  const blocks = getEnabledSkills(preferences)
...
export function buildEmailSkillsPrompt(preferences: string[] = []): string {
  const blocks = getEnabledSkills(preferences)
