...
export function parseStringArray(val: unknown, valid?: readonly string[]): string[] {
  if (!Array.isArray(val) || val.length === 0) return [];
  const arr = val.filter((v) => typeof v === 'string') as string[];
  if (valid) return arr.filter((v) => valid.includes(v) || v === 'MISSION_DOCS');
  return arr;
}

export function normalizeIncomeSources(val: unknown): string[] | { error: string } {
  const list = parseStringArray(val, VALID_INCOME_SOURCES);
  if (list.length === 0) return { error: 'Select at least one income source' };
  return list;
}

export function normalizeAssistancePreferences(val: unknown): string[] {
  const raw = parseStringArray(val, VALID_ASSISTANCE_IDS);
  return normalizeSkillIds(raw);
}

export function normalizeIncomeRange(
  lower: unknown,
  upper: unknown
): { lower: number; upper: number } | { error: string } {
