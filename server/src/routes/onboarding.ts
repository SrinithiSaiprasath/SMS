...
const LIVING_SITUATIONS = ['PG_RENTAL', 'RENTED_APARTMENT', 'OWN_HOME', 'FAMILY_HOME'] as const;
...
router.post('/complete', requireAuth, async (req: AuthRequest, res) => {
  ...
  const {
    ...
    income_lower_inr,
    income_upper_inr,
    agent_assistance_preferences,
  } = data as Record<string, string | number | string[]>;
  ...
  if (!LIVING_SITUATIONS.includes(living_situation as typeof LIVING_SITUATIONS[number])) {
    return res.status(400).json({ error: 'Invalid living situation' });
  }
  if (!FREQUENCIES.includes(report_frequency as typeof FREQUENCIES[number])) {
  ...
  const incomeRange = normalizeIncomeRange(
    income_lower_inr ?? 5000,
