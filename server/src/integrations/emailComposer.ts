...
import type { AgentAOutput } from '../types/index.js';
import type { EmailSection } from '../agents/openaiAgents.js';
...
export function composeMissionEmailHtml(input: {
  ...
  emailSections: Record<string, EmailSection>;
  reportUrl: string;
}): string {
  ...
  const skills = getEnabledSkills(profile.agent_assistance_preferences).filter(
    (s) => s.id !== 'EMAIL_MISSION_REPORTS'
  );
  ...
  if (!sections.EXPENSE_COACHING) {
    sections.EXPENSE_COACHING = {
      ...
      bullets: [
        ...
        ...agentAOutput.saving_opportunities.slice(0, 2).map(
          (o) => `${o.title}: save ~₹${o.estimated_savings_inr.toLocaleString('en-IN')}`
        ),
      ],
    };
  }
